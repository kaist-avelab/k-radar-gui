...
def remove_false_alarms(current_label_path, previous_label_path, next_label_path, is_bus_or_truck=False, thr=0.1,lenght_factor=6,width_factor=1.1,is_plot_result=False):
    ...
    if is_plot_result:
        ...
        plot_2d_boxes(ax, current_objects, 'green', size=2)             # plot current objects in blue
        plot_2d_boxes(ax, previous_objects, 'red', size=1, alpha=0.5)   # plot previous objects in green
        plot_2d_boxes(ax, next_objects, 'blue',size=1, alpha=0.5)       # plot next objects in red


    # adjust the dimension of objects
    ...
    # compute IOU and remove  false alarm
    check_list = []
    ...
    # remove objects which of IOU is lower 0.1
    filtered_objects = []
