...
def post_processing_nms(pred_dicts): 
    try:
        x = pred_dicts['pred_boxes'][:, 0]  
        y = pred_dicts['pred_boxes'][:, 1]  
        l = pred_dicts['pred_boxes'][:, 3]  
        w = pred_dicts['pred_boxes'][:, 4]  
        ...
        boxes_2d = torch.stack((x1, y1, x2, y2), dim=1).squeeze(0) 

        scores = pred_dicts['pred_scores'].squeeze(0)
...
def vis_rdr_data_using_o3d(dict_item, dataset_loaded, predicted_objs, filtered_cls, is_3D_label_vis=False, is_3D_inference_vis=False, is_vis_rdr_points=False):    
    ...
    if is_vis_rdr_points:
        ...
        light_gray_color = [0.75, 0.75, 0.75]  
...
def generate_label_with_LODN_inference_v2(output, filepath = None): 
    if os.path.exists(filepath):
        print("The auto-labeling result has already existed.")
        return
    else:
        data_to_save = []
        try:
        ...
        print('label save path : ', filepath)
